...
class SnapQcRapier(models.Model):
    _name = 'snap.qc.rapier'
    _description = 'Snap Qc Rapier'
    
    
    putus_lusi = fields.Integer('Putus Lusi')
    putus_pakan = fields.Integer('Putus Pakan')
    ambrol_dedel_rantas = fields.Integer('Ambrol/Dedel/Rantas/Pick Finding')
    naik_beam = fields.Integer(string="Naik Beam Baru")
    hb = fields.Integer('Habis Beam')
    pick_finding = fields.Integer('Pick Finding')
    ...
    mesin_kosong = fields.Integer('Mc Kosong(Tunggu Part)')
    ...
    total_mesin = fields.Integer(string="Total Mesin",default=63,readonly=True)
    ...
    def tambah(self):
        for tam in self:
            if tam.total_mesin:
                tam.total_snap = tam.putus_pakan + tam.putus_lusi + tam.ambrol_dedel_rantas + tam.naik_beam + tam.hb + tam.troble_mekanik + tam.troble_elektrik + tam.troble_kualitas +tam.tunggu_beam_cucuk + tam.lain_lain
            else:
                tam.total_snap = 0
